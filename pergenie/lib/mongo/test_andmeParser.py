...
class SimpleTest(unittest.TestCase):
    ...
    def test_success(self):
        p = andmeParser(open(os.path.join('test', 'test.23andme.txt'), 'r'))
    ...
    def test_invalid_file(self):
        ...
        with self.assertRaises(andmeParseError) as cm:
            p = andmeParser(open(os.path.join('test', 'test.vcf41.vcf'), 'r'))
    ...
    def test_invalid_header_1(self):
        ...
        with self.assertRaises(andmeParseError) as cm:
            p = andmeParser(open(os.path.join('test', 'test.23andme.invalid-header.1.txt'), 'r'))
    ...
    def test_invalid_header_2(self):
        ...
        with self.assertRaises(andmeParseError) as cm:
            p = andmeParser(open(os.path.join('test', 'test.23andme.invalid-header.2.txt'), 'r'))

...
def import_catalog(source, catalog_id):
    ...
    fields = [('added',                          'Date Added to Catalog',          parser._date),
              ...
              ('initial_sample_description',     'Initial Sample Description',     str),
              ('replication_sample_description', 'Replication Sample Description', str),
              ('region',                         'Region',                         str),
              ('chr_id',                         'Chr_id',                         int),
              ('chr_pos',                        'Chr_pos',                        int),
    ...
    with open(source, 'rb') as fin:
        for i,record in enumerate(csv.DictReader(fin, delimiter='\t')):
            ...
            for key, key_orig, converter in fields:
                data[key] = parser._type(converter, record[key_orig].strip())

            data['pubmed_link'] = extra.pubmed_link(data['pubmed_id'])
            data['population'] = extra.population(data['initial_sample_description'])
            data['ref'] = extra.reference_allele(data['chr_id'], data['chr_pos'])
            data['OR'], data['beta'] = extra.identfy_or_or_beta(data['OR_or_beta'], data['CI_95'])
